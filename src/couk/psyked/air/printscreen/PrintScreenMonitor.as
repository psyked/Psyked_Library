        ...
        private function onTimerEvent( e:TimerEvent ):void
        {
            ...
            else
            {
                if ( bitmapfound )
                {
                    ...
                    {
                        var latestBMD:BitmapData = Clipboard.generalClipboard.getData( ClipboardFormats.BITMAP_FORMAT ) as BitmapData;
                        
                        if ( latestBMD != null )
                        {
                            if ( bitmapdatacache.compare( latestBMD ) == 0 )
                            {
                            ...
                            else
                            {
                                ...
                                outEvent.bitmap = new Bitmap( latestBMD )
                                dispatchEvent( outEvent );
                            }
                        }
                        else
                        {
                            // if there's a problem converting the clipboard data to a bitmap, do nothing.
                        }
                    }
                }
                else
